...
```
자동 검증 가능 (사람 불필요)
  후보에 facts 가 실렸는가            상품 18건 · 후보 420건 전부 OK (420/420)
  판정이 실행됐는가                    후보 420건 전부 OK (판정 미실행 0건)
  판정 분포                            CONFLICT 346 / SIMILAR 40 / SAME 18 / UNKNOWN 11 / PRESUMED_SAME 5
  원본 상품 자신을 SAME 으로 맞히는가   자기 판매처가 검색된 11건 전부 OK
  이미지 URL 이 실제로 열리는가         아래 목록 전부 200 image/*

사람이 봐야 하는 것 - 딱 두 묶음, 총 12건
  (1) 아래 7-2 의 거짓 SAME 7건  (CEO 판단이 필요한 정책 질문 1개로 압축된다)
  (2) PRESUMED_SAME 5건(#14 Mini Rodini 2건 · #16 folkberlin Liewood 3건)
      - 참고 등급이라 가격 판단에 쓰이지 않는다. 급하지 않다
```

### 7-2. 데이터셋이 드러낸 것 - **거짓 SAME 7건, 원인은 전부 하나**

SAME 18건 중 **11건은 "원본 상품 자신"**(원본 판매처가 검색 대상에 있으니 자기
상품을 찾은 것 — 정상이고, 판정기가 옳게 동작한다는 증거이기도 하다). 나머지
**7건은 전부 거짓**이고, 원인은 하나다 — 판매처가 여러 상품에 같은 `Product Code`를
적는다(§3-4).
