...
## 🟠 P1 — 신뢰도 0.1 자동채움이 「확인 필요 0개」로 집계된다

**기록 경위**: CEO 지시(2026-09-22) — COUPANG-REAL-OPTION-01 조사 중 드러났으나
**그 수정에 섞지 않고 별도 P1 로 분리**한다. 이건 payload 버그가 아니라 «등록 전
사전 경고 체계» 의 문제다.

### 실측

```text
registration_attempts a7572b88  ·  2026-09-22 10:55  ·  쿠팡 LIVE 실패(API005)

complianceFieldResults
  색상  source=PLACEHOLDER  confidence=0.1  critical=false  unmappedReason=NO_VALUE
...
신뢰도 **0.1** 짜리 자리채움이 「자동 입력」으로 «세어졌고», 확인 필요는 0개였다.
셀러는 15분 4초를 입력하고 나서야 실패를 봤다.

### 무엇이 문제인가

`PLACEHOLDER` 는 이름 그대로 «사람이 확인해야 한다» 는 뜻인데, 화면 집계가
`source` 나 `confidence` 를 보지 않고 「값이 있으면 채워진 것」으로 센다.
`critical` 은 `isComplianceCritical(fieldName)`(KC/인증 계열 이름 포함 여부)로만
결정돼서 색상·사이즈 같은 구매옵션은 언제나 false 다.

### 고칠 때 주의

```text
❌ PLACEHOLDER 를 무조건 「확인 필요」로 올리면 기존에 정상 등록되던 상품까지
   전부 경고가 붙을 수 있다 — 범위를 먼저 재야 한다.
❌ confidence 임계값을 임의로 정하지 않는다(0.1/0.3/0.5 중 무엇인지 근거 필요).
🔴 COUPANG-REAL-OPTION-01 의 재등록 결과를 본 뒤에 착수한다 — 그 결과가
   「어떤 PLACEHOLDER 가 실제로 등록을 죽이는가」를 알려준다.
```
