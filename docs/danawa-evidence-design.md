...
⑥ 판매처 성격 판별 불가 상태에서의 사용 여부  →  **[POLICY] 확정됨 (아래)**
```

### 🔴 [POLICY] — CEO 확정, 2026-09-21

> **`DANAWA_PRICE_COMPARISON` 은 현재 판매판정에 직접 사용하지 않는다.
> 기존 EXACT-only 국내가격 정책을 유지하며, Danawa 가격비교 결과는 별도 Evidence 로
> 보존·표시할 수 있는 참고 가격정보로 취급한다. 향후 판매판정 사용 여부는
> 판매처 성격·구매가능성·가격조건·coverage 에 대한 추가 근거가 확보된 후 별도 결정한다.**

확정 근거 (전부 §1~§5 의 실측·미확인 항목):

```text
· 특정 판매처의 실제 판매가격이 아니라 «가격비교 최저가» 다
· 배송비가 별도일 수 있다 (data-delivery-price)
· 최저가 판매처의 성격을 일관되게 판별할 수 없다   [UNVERIFIED]
· 품절/판매중지 여부가 현재 데이터로 검증되지 않는다 [UNVERIFIED]
· 다나와가 국내 시장 전체를 대표한다고 볼 근거가 없다
· 브랜드별 pcode 확보율이 편중되어 있다
```

🔴 이 값을 `domesticBasis === "EXACT"` 에 넣으면 **이미 해결한 EXACT/COMPARISON 의미를
다시 흔든다**(정책 A · P0-D.2 에서 확정한 것). 그래서 넣지 않는다.

### 🔴 후속 원칙 — 낮은 가격을 «시장가» 로 자동 해석하지 않는다

> 다나와가 낮은 가격을 보여준다고 해서 「국내 실제 경쟁가격도 낮다」고 자동 판단하지 않는다.

**출처와 의미를 그대로 보여주고 판단은 셀러가 한다.**

```text
국내 동일상품      FORETFORET      ₩162,000     ← 판매판정 기준
다나와 가격비교     최저가           ₩109,500     ← 참고 · 교차검증
```
...
B  현재 판매판정에 직접 쓸 수 있는가?        🔴 아니다 — §6 · [POLICY] 로 확정됨
C  쓰려면 최소 조건은?                      §7 ①~⑤ · ⑥ 은 «현재 미사용» 으로 확정
...
```text
DANAWA-06   설계 완료 · ✅ 정책 «확정» (§7 [POLICY], CEO 2026-09-21)
구현 착수    🔴 HOLD — DANAWA adapter · PRICE_COMPARISON 구현 · 판매판정 연결 전부 HOLD
             기존 국내소스 «유지» · 신규 국내소스 조사 HOLD
코드 0 · DB 0 · migration 0 · Production 0
```

✅ **§7 ⑥ 은 확정됐다**(CEO, 2026-09-21) — 판매판정 **직접 사용 안 함**.

🔴 **나머지는 여전히 [REC] 다.** 독립 축 구성 · 배치 구조 · stale 기준 · UI 배치는
설계안이며 구현하지 않는다. **Danawa 는 버리지 않는다** — 참고·교차검증 레이어로 유지한다.
