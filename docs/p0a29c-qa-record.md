...
### 2차 관측 (P0-A.29-D, 같은 날)

같은 증상이 다시 나왔고, **이번에도 이름을 잡지 못했다.**

```
총 14회 실행  →  2회에서 「1 file failed」
그중 파일로 출력을 받아 적은 12회는 전부 통과
실패한 2회는 둘 다 «파이프로 grep 에 흘려보낸» 실행이었다
```

- SIGPIPE(`| head` 가 파이프를 먼저 닫아 vitest 의 stdout 쓰기가 깨진다) 를
  의심해 `| head` 를 붙여 3회 재시도했으나 **재현되지 않았다.**
  → 이 가설도 «확인되지 않음» 으로 남긴다.
- 깨진 파일 이름을 끝내 얻지 못했다. 실패 요약이 파이프 뒤로 잘려 나갔다.

**다음에 볼 사람에게:** 재현을 노릴 때는 출력을 파이프에 흘리지 말고
파일로 받아라(`pnpm vitest run > out.txt 2>&1`). 그래야 실패 요약이 남는다.

**이 문단을 지우는 조건은 원인을 특정했을 때뿐이다.**
