...
export async function POST(request: Request) {
  ...
  /* 🔴 마지막 빗장 — SmartStore route 와 같은 이유로 둔다. 여기서부터 아래는
     ...
     기대지 않는다. */
  /* 🔴 마지막 빗장 — SmartStore·LotteON 과 «같은 함수» 가 정한다(F-12a).
     쿠팡에도 중복 3건이 이미 있고(16336681622 · 16338809221 · 16340176952),
     그중 일부가 「연결은 없는데 이미 나가 있는」 경로로 생겼다.
     🔴 문구만 채널별로 다르고 «판단은 하나» 다 — 세 벌로 두면 갈라진다. */
  const gate = resolveCreateGate({
    hasChannelProduct: Boolean(existing),
    priorSuccess:
      plannedOperation === "RECREATE" ? false : await hasPriorSuccessfulAttempt(snapshotId, "coupang"),
    plannedOperation,
  });
  if (gate !== "ALLOW") {
