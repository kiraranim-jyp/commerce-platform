 ...
 */
/**
 * ── MI-MARKET-EVIDENCE-1(CEO 지시, 2026-09-12) — 두 시장이 제목으로 갈린다 ─────
 * "한국 시장 경쟁가격"은 국내 블록 하나만 있던 시절의 이름이다. 이제 본문에는
 * 국내와 해외가 나란히 서고, 둘은 **서로 다른 질문**에 답한다:
 *
 *   🇰🇷 국내 시장   한국에서 경쟁 가능한가(다른 한국 판매자들의 실제 판매가)
 *   🌎 해외 시장   이 상품이 해외에서 어떤 가격대인가(다른 해외 판매처들의 가격)
 *
 * 그래서 이름도 대칭으로 둔다 — 한쪽만 "경쟁가격"이면 다른 쪽은 무엇인지 제목이
 * 말해주지 못하고, 셀러는 둘을 같은 종류의 숫자로 읽는다. 국기를 다시 붙인
 * 이유도 같다: 이 화면에서 두 블록을 가르는 사실이 정확히 "어느 나라인가"다
 * (MI-FINAL-UX-3이 이모지를 뗀 것은 제목 셋이 전부 같은 층의 명사였을 때다).
 */
