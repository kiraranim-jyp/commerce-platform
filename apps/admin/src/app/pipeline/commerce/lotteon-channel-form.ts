...
const LOTTEON_FIX_GUIDE: Record<string, Omit<LotteOnMissingInfoItem, "key" | "label" | "blocking">> = {
  ...
  /* ══ Commerce-6 C-2F(CPO 지시, 2026-09-26) ══
     C-2E 가 `itmStkQty`(재고) 판정을 새로 만들었는데 이 표에 자리가 없어
     FALLBACK 으로 떨어졌다 — 그러면 화면이 「롯데ON 탭에서 채우세요」라고
     말한다(FALLBACK 의 where 가 LOTTEON_TAB 이다).

     🔴 CPO 가 말한 오류 유형 C 다: 셀러가 여기서 해결할 수 «없는» 원본의
     사실을 이 탭에서 입력하라고 안내하는 것. 재고는 상품정보에서 확인하거나
     원본 재입고를 기다릴 일이지 롯데ON 탭에서 적을 값이 아니다. */
  itmStkQty: {
    why: "원본 상품에 재고가 없으면 팔 수 없는 물건을 올리게 됩니다.",
    what: "상품정보 탭에서 원본 재고를 확인해 주세요. 원본이 품절이면 재입고 뒤에 등록할 수 있습니다 — 판매 여부와 판매가격은 시장 정보를 보고 직접 판단하시면 됩니다.",
    where: "COMMON_PRODUCT",
  },
  slPrc: {
