...
describe("GOLF-03 — 다섯 축 동일상품 비교", () => {
  ...
  it("어느 국내축을 기준으로 잡아도 판매자 원가가 더 높다 — 축을 바꿔도 후보가 되지 않는다", () => {
    for (const a of AXES) {
      ...
      expect(bestDomestic - bestCost, a.label).toBeLessThan(0);
    }
  });

  it("경량 상품에서는 국제배송비 한 칸이 국내 두 축의 차이보다 크다 — 결론을 지배하는 것은 배송비다", () => {
    ...
    // 국내 두 축의 최대 격차(₩22,568)조차 EMS 2kg 이하 한 칸(₩29,813)보다 작다.
    expect(shippingKrw).toBeGreaterThan(Math.max(...domesticSpread));
  });
