...
export function validateNaverPayload(
  ...
  if (notice === "kids") {
    ...
    check(
      ...
      "MISSING",
      // DELTA-B(CEO 판정, 2026-09-15) — 이 줄과 아래 naverShoppingSearchInfo.
      // modelName은 **서로 다른 필드**다. 둘 다 "모델명이 없습니다"로 말하면
      // 셀러는 한 항목을 두 번 본 줄 알고 하나만 처리한다.
      "고시정보 모델명이 없습니다 — 상품 정보 편집 화면에서 직접 입력하거나 \"상세페이지 참조\"로 대체할 수 있습니다(스마트스토어 등록을 막는 「네이버 쇼핑 카탈로그 모델명」과는 별도 값입니다).",
    );
    ...
    if (!originProduct.detailAttribute?.naverShoppingSearchInfo?.modelName) {
      blocked(
        ...
        // REWORK-5 ②(CEO 지시, 2026-09-14) — 이 문장이 우선순위 카드의 "왜
        // 필요한가 / 현재 상태"로 그대로 나간다. 마지막 문장이 특히 중요하다:
        // 화면의 "모델명" 입력칸에는 「상세페이지 참조로 등록」 버튼이 나란히
        // 붙어 있는데, 그 버튼을 누르면 값이 비고 source가 DETAIL_PAGE_REFERENCE가
        // 되어 build-payload.ts가 이 필드를 **끝내 채우지 않는다**(USER_EDITED일
        // 때만 쓴다 — 카탈로그 매칭용 값이라 "상세페이지 참조"라는 문구를 넣을
        // 수 없다). 그 사실을 안 적어 두면 셀러는 안내를 따랐는데도 차단이
        // 풀리지 않는 것을 보게 된다.
        // DELTA-B(CEO 지시, 2026-09-15) — **경고 시점을 앞으로.** 기존 문장은
        // "…에 필요한 값입니다"로 시작해 셀러가 이미 막힌 뒤에야 두 모델명이
        // 다르다는 것을 알게 했다. 첫 문장에서 (1) 무엇이 필요한지 (2) 고시정보
        // 모델명과 별도 값이라는 것을 먼저 말한다.
        "네이버 쇼핑 카탈로그 모델명이 필요합니다 — 상세페이지 고시정보의 모델명과는 별도 값입니다. 네이버 쇼핑 카탈로그 검색·연결에 쓰이는 값이라 \"상세페이지 참조\"로는 대체할 수 없습니다. 현재 상태: 상품 원문에서 모델명(Product code 표기)을 찾지 못했습니다 — 「기본정보」의 \"모델명\"(상품정보 화면에서는 \"네이버 쇼핑 카탈로그 모델명\")에 직접 입력해야 합니다.",
        "KC_CERTIFICATION_REQUIRED",
      );
    }
  }
